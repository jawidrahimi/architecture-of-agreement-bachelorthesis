...
# CONFIGURATION
# ==========================================
CSV_FILE = "group_level_prevalences.csv"

# Map the 20 Level 2 values to the 4 Higher-Order Foci
FOCI_MAPPING = {
    "Openness to Change": [
...
# Helper to reconstruct column names matching quantify_groups.py
...
if not os.path.exists(CSV_FILE):
    raise FileNotFoundError(f"Could not find CSV file: {CSV_FILE}. Run quantify_groups.py first.")
...
for _, row in df.iterrows():
    # Construct 4D foci vectors for OP, Win, and Lose
    op_vector = []
    win_vector = []
    lose_vector = []
    
    for focus, features in FOCI_MAPPING.items():
        # Represent coordinate in focus space as the mean prevalence of its sub-values
